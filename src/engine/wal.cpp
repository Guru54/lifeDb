...
void Wal::appendEntry(const Entry& e) {
    ...
    if (e.op == Op::Insert) {
        ...
        size_t off = 0;
        memcpy(payload.data() + off, &tlen, 2); off += 2;
        memcpy(payload.data() + off, e.table.data(), tlen); off += tlen;
        memcpy(payload.data() + off, &rlen, 4); off += 4;
...
void Wal::logCommit(uint64_t txn_id) {
    Entry e; e.txn_id = txn_id; e.op = Op::Commit;
    appendEntry(e);
}

void Wal::logRollback(uint64_t txn_id) {
    Entry e; e.txn_id = txn_id; e.op = Op::Rollback;
    appendEntry(e);
}
