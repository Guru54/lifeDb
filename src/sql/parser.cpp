...
void Parser::expect(TokenKind k, const char* msg) {
  if (cur_.kind != k) throw std::runtime_error(msg);
  advance();
}

std::string Parser::parseIdent() {
  if (cur_.kind != TokenKind::Identifier) throw std::runtime_error("expected identifier");
  auto s = cur_.text;
  advance();
  return s;
}
...
DataType Parser::parseType() {
  if (match(TokenKind::KW_INT)) return DataType::Int;
  if (match(TokenKind::KW_TEXT)) return DataType::Text;
...
Stmt Parser::parseStatement() {
  if (match(TokenKind::KW_BEGIN)) { match(TokenKind::Semicolon); return {StmtKind::Begin, std::monostate{}}; }
  if (match(TokenKind::KW_COMMIT)) { match(TokenKind::Semicolon); return {StmtKind::Commit, std::monostate{}}; }
  if (match(TokenKind::KW_ROLLBACK)) { match(TokenKind::Semicolon); return {StmtKind::Rollback, std::monostate{}}; }
